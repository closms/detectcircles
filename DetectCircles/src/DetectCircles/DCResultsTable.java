...
*/
public class DCResultsTable implements Cloneable {
	...
	//public static DCResultsTable getResultsTable() {
	...
	*/
	...
	public static DCResultsTable createTableFromImage(ImageProcessor ip) {
		DCResultsTable rt = new DCResultsTable();
	...
	*/
	...
	public synchronized int findRow(double x, double y, double d, double s) {
		// This is a real hack.
		for (int rowix = 0; rowix < counter; rowix++) {
			if (columns[0][rowix] == x &&
					columns[1][rowix] == y &&
					columns[2][rowix] == d &&
					columns[3][rowix] == s) {
				return rowix;
	...
	public void updateResults() {
		Frame frame = WindowManager.getFrame(this.windowTitle);
		DCTextWindow win;
		if (frame!=null && frame instanceof DCTextWindow) {
			win = (DCTextWindow)frame;
		}
		else {
			return;
		}
		DCTextPanel textPanel = win.getTextPanel();
		if (textPanel!=null) {
	...
	public void show() {
		...
		//} else {
			...
			if (frame!=null && frame instanceof DCTextWindow)
				...
				win = new DCTextWindow(this.windowTitle, "", 400, 300);
				cloneNeeded = true;
			}
			tp = win.getTextPanel();
			...
			autoFormat = false;
		//}
		tp.setResultsTable(cloneNeeded?(DCResultsTable)this.clone():this);
		int n = getCounter();
		if (n>0) {
...
//		DCResultsTable rt2 = new DCResultsTable();
...
//	}
	...
	public synchronized Object clone() {
		try { 
			DCResultsTable rt2 = (DCResultsTable)super.clone();
