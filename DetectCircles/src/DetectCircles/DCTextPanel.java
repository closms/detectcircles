...
*/
public class DCTextPanel extends Panel implements AdjustmentListener,
	...
	/** Constructs a new TextPanel. */
	public DCTextPanel(String title) {
		this();
	...
	*/
	...
	void rename(String title2) {
		...
		if (comp==null || !(comp instanceof DCTextWindow))
			return;
		DCTextWindow tw = (DCTextWindow)comp;
		if (title2==null) {
		...
		if (iColCount==1 && tc.fMetrics!=null && r>=0 && r<iRowCount) {
      	...
      	if(r>=0 && r<iRowCount && x<lineWidth) {
			...
			selEnd = r;
      		...
      		/* Find the column that the user clicked. */
      		int c, X = 0;
      		for (c = 0; c < iColWidth.length; c++) {
      			X += iColWidth[c];
      			if (x < X) {
      				/* Got it. */
      				break;
      		...
      		if (c < iColWidth.length) {
      			/* Toggle sorting on column c. Only partially implemented at this point. */
      			//IJ.log("Toggle sorting on column "+c);
      		}
      	}
      	else {
			resetSelection();
	...
	*/
	...
	public void setResultsTable(DCResultsTable rt) {
		this.rt = rt;
	}
	
	/** Returns the ResultsTable associated with this TextPanel, or null. */
	public DCResultsTable getResultsTable() {
		return rt;
	}
